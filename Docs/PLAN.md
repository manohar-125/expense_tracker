...
Bottom tabs (exactly three): **Home** (dashboard), **Add** (center, larger than the other two tabs), **More**. Home and More are root screens with no back button. Tapping Add does not change the selected tab; it opens a tall centered fade-in add-expense popup on the current screen. After save, a centered fade-in confirmation popup lists every entered field as two columns (label left, value right). Short labels and values stay on one line; long description text wraps in the value column. Done returns to Home. Home has an outline history icon in the title row that opens the Expenses list.
